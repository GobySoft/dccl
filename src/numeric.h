...
/// decomposes float/double to a two-integer representation: a significand (aka mantissa)
/// and exponent. The functionality is similar to std::frexp except the significand is
/// represented as an integer type instead of a floating point representation.
/// Conceptually: val = <return value> * 2 ^ exponent
///
/// \param val value to decompose
/// \param exponent that allows the significand to be expressed without a decimal point
...
template <std::size_t N> inline void increment(std::bitset<N>& bits)
{
...
template <std::size_t N> inline bool is_negative(const std::bitset<N>& bits)
{
    return bits[N - 1];
}

template <std::size_t N> inline void negate(std::bitset<N>& bits)
{
    bits.flip();
    increment(bits);
}

template <std::size_t N> inline std::bitset<N> negated(const std::bitset<N>& bits)
{
    auto ret_val = bits;
    negate(ret_val);
    return ret_val;
...
template <std::size_t N> inline void add_to(std::bitset<N>& a, const std::bitset<N>& b)
{
    bool carry_bit = false;
    for (auto i = 0ul; i < N; ++i)
    {
        auto s = carry_bit + a[i] + b[i];
        a[i] = s % 2;
        carry_bit = s > 1;
    }
}

template <std::size_t N> inline std::bitset<N> sum(const std::bitset<N>& a, const std::bitset<N>& b)
{
    auto ret_val = a;
    add_to(ret_val, b);
    return ret_val;
...
inline std::bitset<2 * N> unsigned_product(const std::bitset<N>& a, const std::bitset<N>& b)
{
    ...
    for (auto b_i = 0ul; b_i < N; ++b_i)
    {
        ...
        {
            auto s = p[a_i + b_i] + carry + (a[a_i] * b[b_i]);
            carry = static_cast<bool>(s >> 1);
            p[a_i + b_i] = static_cast<bool>(s % 2);
        }
        p[b_i + N] = carry;
    }
...
template <typename T, std::size_t N> inline T fill_unsigned(const std::bitset<N>& bits)
{
    auto ret_val = T{};
...
template <typename Integral,
          typename std::enable_if<std::is_integral<Integral>::value, bool>::type = true>
...
Integral decode(uint64 value_enc, double min, double res)
{
    ...
    wire_value = quantize(wire_value + quantize(static_cast<Integral>(min), res), res);
    return wire_value;
}

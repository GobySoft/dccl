...
\label{tab:dccl_enc}
\begin{tabular}{p{0.15\textwidth}|p{0.28\textwidth}|p{0.5\textwidth}}
...
\begin{array}{l l}
  ...
  0 & \quad \text{otherwise} \\
\end{array} \right.$   \\ 
\hline string (of length $L$) & $8 + \text{min}(L, L_M) \cdot 8$ & $x_{enc} = L + \sum_{n=0}^{\text{min}(L, L_M)} x[n] \cdot 2^{8(n+1)}$ \\ 
\hline bytes & $L_M \cdot 8$ & $x_{enc}$ = $x$ \\ 
\hline \textit{Message} & sum of \textit{Message} field sizes & $x_{enc}$ for each field of \textit{Message} bitwise concatenated (recursive encoding).  \\ 
...
\begin{array}{l l}
  ...
  0 & \quad \text{otherwise} \\
\end{array} \right.$   \\ 
\hline string & \multicolumn{2}{c}{same as \textit{required}} \\ 
\hline bytes & $L_M \cdot 8 + 1$ & $x_{enc} =  \left\{ 
\begin{array}{l l}
  x \cdot 2 + 1 & \quad \text{if $x$ is set }\\
  0 & \quad \text{otherwise} \\
\end{array} \right.$   \\
\hline \textit{Message} & \multicolumn{2}{c}{same as \textit{required}} \\ 
\hline \multicolumn{3}{c}{\textit{repeated} fields} \\ \hline
\hline all & $r_M \cdot q_{optional}$ & $x_{enc}$ for \textit{optional} field is calculated for each repeated element then bitwise concatenated (where repeated index 0 is at the least significant bit)  \\ 
\hline
\end{tabular}
\begin{tablenotes}
